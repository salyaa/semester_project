...
def validation_range_K(range_K, nb_probas: int=5, modify: str="out"):
    ...
    name_table = f"evaluations/scores_K{range_K[0]}to{range_K[-1]}_n{nb_probas}"
    results.to_csv(name_table)
    
    return results


def plot_results_range_K(results):
    algos = results.columns  # Clustering algorithms
    indexes = results.index.get_level_values("Metric").unique()  # Unique evaluation metrics
    range_K = results.index.get_level_values("K").unique()  # Unique K values

    # Create subplots
    fig, axs = plt.subplots(1, len(indexes), figsize=(5 * len(indexes), 5), sharex=True, sharey=True)

    # Ensure axs is always an array (for single-metric case)
    if len(indexes) == 1:
        axs = [axs]

    for i, ind in enumerate(indexes):
        avg_scores = results.xs(ind, level="Metric")  # Extract rows for a specific metric

        for algo in algos:
            axs[i].plot(range_K, avg_scores[algo], marker='o', label=algo)

        # Plot settings
        axs[i].set_title(ind.replace("_", " ").title())  # Format metric name
